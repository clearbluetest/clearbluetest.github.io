(function() {
	$('#slider').on('ready', '.Clearblue_021a', function(e, el) {
		var slide = $('#container', el);

	/*pop16*/	
			if(isAnimation)
		{
			$('.drugs, .txt2_br, .condom,.txt1_br',slide).fadeIn(function(){
				$('.txt3_br, .txt8_br',slide).fadeIn(function(){
					$('.txt4_br,.txt9_br',slide).fadeIn(function(){
						$('.txt5_br,.txt10_br',slide).fadeIn(function(){
							$('.txt6_br,.txt11_br',slide).fadeIn(function(){
								$('.txt7_br,.txt12_br',slide).fadeIn();
							});
						});
					});
				});
